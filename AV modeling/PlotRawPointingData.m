...
subjectID = 'RJ';
session = 'Visual_Capture_Session3';
sessionString = 'Visual Capture Session 3';
...
set(gcf, 'PaperPosition',[0 0 11 12])
...
plot(errorSplinePoints(1,:),errorSplinePoints(2,:),'--','color',[0.6 0.6 0.6]);
...
set(gcf, 'PaperOrientation','portrait')
print('-dpng',['Figures\' subjectID ' ' sessionString ' pointing Error DeltaAV with Spline.png']);
...
plot([-1*maxDisparity maxDisparity], [maxDisparity -1*maxDisparity]*(SDA^2/(SDA^2+SDV^2)),'-','LineWidth',2,'color',integrationColor);
%Plot a spline with confidence interval for the raw data
plot(correctedSplinePoints(1,:),correctedSplinePoints(2,:),'--','color',[0.6 0.6 0.6]);
%plot the raw pointing data
...
set(gcf, 'PaperPosition',[0 0 11 6])
set(gcf, 'PaperOrientation','portrait')
print('-dpng',['Figures\' subjectID ' ' sessionString ' corrected Error DeltaAV with Spline.png']);
