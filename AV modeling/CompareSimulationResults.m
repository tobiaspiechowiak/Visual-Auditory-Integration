...
fixedParamNames = fieldnames(subjectData(1).Sessions(1).meanFitResult.fixedParameters(1));
for(fixedParameterIndex = 1:length(fieldnames(subjectData(1).Sessions(1).meanFitResult.fixedParameters)))
	paramValues = [];
	paramConfInt = [];
	sessionType = [];
	sessionNumber = [];
	modelType = cell(0,0);
	subjectID = cell(0,0);
	paramName = fixedParamNames{fixedParameterIndex};
	for(subjectIndex = 1:length(subjectData))
		for(fileNumber = 1:length(subjectData(subjectIndex).Sessions))
			...
			if(isstruct(subjectData(subjectIndex).Sessions(fileNumber).meanFitResult))
				...
				paramValues = [paramValues...
					eval(['subjectData(subjectIndex).Sessions(fileNumber).meanFitResult.fixedParameters.' paramName])];
	...
	%Display the mean values for the parameter
	disp(' ');
	disp(' ');
	disp('********************************************************************************');
	disp(['Median value for ' paramName ' = '...
       			num2str(median(unique(paramValues)))]);
	disp(['Medians split by task type:']);
	disp(['     Binding: ' num2str(median(unique(paramValues(sessionType))))]);
	disp(['     Visual Capture: ' num2str(median(unique(paramValues(~sessionType))))]);
	disp(['Median Range: ' num2str(min(paramValues)) ' - ' num2str(max(paramValues))]);
	disp(['Meidan Ranges split by task type:']);
	disp(['     Binding: ' num2str(min(paramValues(sessionType))) ' - ' num2str(max(paramValues(sessionType)))]);
	disp(['     Visual Capture: ' num2str(min(paramValues(~sessionType))) ' - ' num2str(max(paramValues(~sessionType)))]);
end
...
for (parameterIndex = 1:length(subjectData(1).Sessions(1).meanFitResult.freeParameters))
	...
	paramRange = paramConfInt(2,:) - paramConfInt(1,:);
	...
	disp(['     Binding: ' num2str(min(paramValues(sessionType))) ' - ' num2str(max(paramValues(sessionType)))]);
	disp(['     Visual Capture: ' num2str(min(paramValues(~sessionType))) ' - ' num2str(max(paramValues(~sessionType)))]);
	disp(['Range of Parameter Ranges: ' num2str(min(paramRange)) ' - ' num2str(max(paramRange))]);
	disp(['Range of parameter ranges split by task type:']);
	disp(['     Binding: ' num2str(min(paramRange(sessionType))) ' - ' num2str(max(paramRange(sessionType)))]);
	disp(['     Visual Capture: ' num2str(min(paramRange(~sessionType))) ' - ' num2str(max(paramRange(~sessionType)))]);
	...
	figure;
	%"Best" parameters are plotted bigger in size, and with a solid pattern.  In the event of a tie, both are given the "best" visual style
	errorbar(plotAxis(sessionType & bestInSession),paramValues(sessionType & bestInSession),...
		paramLowerBound(sessionType & bestInSession),paramUpperBound(sessionType & bestInSession),'.k','MarkerSize',20,'LineWidth',3);
	hold on;
	errorbar(plotAxis(~sessionType & bestInSession),paramValues(~sessionType & bestInSession),...
		paramLowerBound(~sessionType & bestInSession),paramUpperBound(~sessionType & bestInSession),...
		'.','Color',[0.5 0.5 0.5],'MarkerSize',20,'LineWidth',3);
	%Parameters that are not "best" are smaller in size
	errorbar(plotAxis(sessionType & ~bestInSession),paramValues(sessionType & ~bestInSession),...
		paramLowerBound(sessionType & ~bestInSession),paramUpperBound(sessionType & ~bestInSession),'.k','MarkerSize',10,'LineWidth',1);
	errorbar(plotAxis(~sessionType & ~bestInSession),paramValues(~sessionType & ~bestInSession),...
		paramLowerBound(~sessionType & ~bestInSession),paramUpperBound(~sessionType & ~bestInSession),...
		'.','Color',[0.5 0.5 0.5],'MarkerSize',10,'LineWidth',1);
	scatter(plotAxis(strcmp(modelType,'Averaging')),paramValues(strcmp(modelType,'Averaging')),70,plotColor(strcmp(modelType,'Averaging'),:),...
		'filled','Marker',averagingMarker);
	scatter(plotAxis(strcmp(modelType,'Matching')),paramValues(strcmp(modelType,'Matching')),70,plotColor(strcmp(modelType,'Matching'),:),...
		'filled','Marker',matchingMarker);
	scatter(plotAxis(strcmp(modelType,'Selection')),paramValues(strcmp(modelType,'Selection')),70,plotColor(strcmp(modelType,'Selection'),:),...
		'filled','Marker',selectionMarker);
	axis([17 226 minParamBound maxParamBound]);
	set(gca,'FontSize',20);
	...
	%print('-dpng',['Figures\paramValues ' cell2str(paramName) ' ' num2str(numParams) ' parameters.png']);
	...
	scatter(paramValues(xindex & ~sessionType & strcmp(modelType,'Averaging')),paramValues(yindex & ~sessionType & strcmp(modelType,'Averaging')),...
		70,plotColor(xindex & ~sessionType & strcmp(modelType,'Averaging'),:),'filled','Marker',averagingMarker,'MarkerEdgeColor',[0.5 0.5 0.5]);
	scatter(paramValues(xindex & ~sessionType & strcmp(modelType,'Matching')),paramValues(yindex & ~sessionType & strcmp(modelType,'Matching')),...
		70,plotColor(xindex & ~sessionType & strcmp(modelType,'Matching'),:),'filled','Marker',matchingMarker,'MarkerEdgeColor',[0.5 0.5 0.5]);
	scatter(paramValues(xindex & ~sessionType & strcmp(modelType,'Selection')),paramValues(yindex & ~sessionType & strcmp(modelType,'Selection')),...
		70,plotColor(xindex & ~sessionType & strcmp(modelType,'Selection'),:),'filled','Marker',selectionMarker,'MarkerEdgeColor',[0.5 0.5 0.5]);
	scatter(paramValues(xindex & sessionType & strcmp(modelType,'Averaging')),paramValues(yindex & sessionType & strcmp(modelType,'Averaging')),...
		70,plotColor(xindex & sessionType & strcmp(modelType,'Averaging'),:),'filled','Marker',averagingMarker,'MarkerEdgeColor','black');
	scatter(paramValues(xindex & sessionType & strcmp(modelType,'Matching')),paramValues(yindex & sessionType & strcmp(modelType,'Matching')),...
		70,plotColor(xindex & sessionType & strcmp(modelType,'Matching'),:),'filled','Marker',matchingMarker,'MarkerEdgeColor','black');
	scatter(paramValues(xindex & sessionType & strcmp(modelType,'Selection')),paramValues(yindex & sessionType & strcmp(modelType,'Selection')),...
		70,plotColor(xindex & sessionType & strcmp(modelType,'Selection'),:),'filled','Marker',selectionMarker,'MarkerEdgeColor','black');
	axis([minParamBound maxParamBound minParamBound maxParamBound]);
	axis square;
	title(paramName);
	set(gca,'YTick',-20:0.5:20);
	set(gca,'XTick',-50:0.5:50);
	set(gca,'FontSize',14);
	ylabel('Session 3 ','fontsize',16);
	xlabel('Session 1 ','fontsize',16);
	...
	scatter(paramValues(xindex & sessionNumber == 2 & strcmp(modelType,'Averaging')),...
		...
		70,plotColor(xindex & sessionNumber == 2 & strcmp(modelType,'Averaging'),:),...
	...
	scatter(paramValues(xindex & sessionNumber == 2 & strcmp(modelType,'Matching')),...
		...
		70,plotColor(xindex & sessionNumber == 2 & strcmp(modelType,'Matching'),:),...
	...
	scatter(paramValues(xindex & sessionNumber == 2 & strcmp(modelType,'Selection')),...
		...
		70,plotColor(xindex & sessionNumber == 2 & strcmp(modelType,'Selection'),:),...
	...
	scatter(paramValues(xindex & sessionNumber == 3 & strcmp(modelType,'Averaging')),...
		...
		70,plotColor(xindex & sessionNumber == 3 & strcmp(modelType,'Averaging'),:),...
	...
	scatter(paramValues(xindex & sessionNumber == 3 & strcmp(modelType,'Matching')),...
		...
		70,plotColor(xindex & sessionNumber == 3 & strcmp(modelType,'Matching'),:),...
	...
	scatter(paramValues(xindex & sessionNumber == 3 & strcmp(modelType,'Selection')),...
		...
		70,plotColor(xindex & sessionNumber == 3 & strcmp(modelType,'Selection'),:),...
	...
	set(gca,'XTick',-50:0.5:50);
	set(gca,'FontSize',14);
	ylabel('Auditory Localization','fontsize',16);
	xlabel('Congruence Judgment','fontsize',16);
	...
	if(coefTest(fitS1S2) < 0.05)
		...
		plot([minParamBound maxParamBound], fitSMAS1S2(1) + fitSMAS1S2(2)*[minParamBound maxParamBound],'k','LineWidth',2);
	end
	...
	scatter(paramValues(xindex & sessionNumber == 1 & strcmp(modelType,'Averaging')),...
		paramValues(yindex & sessionNumber == 2 & strcmp(modelType,'Averaging')),...
		70,plotColor(xindex & sessionNumber == 1 & strcmp(modelType,'Averaging'),:),...
		'filled','Marker',averagingMarker,'MarkerEdgeColor','black');
	scatter(paramValues(xindex & sessionNumber == 1 & strcmp(modelType,'Matching')),...
		paramValues(yindex & sessionNumber == 2 & strcmp(modelType,'Matching')),...
		70,plotColor(xindex & sessionNumber == 1 & strcmp(modelType,'Matching'),:),...
		'filled','Marker',matchingMarker,'MarkerEdgeColor','black');
	scatter(paramValues(xindex & sessionNumber == 1 & strcmp(modelType,'Selection')),...
		paramValues(yindex & sessionNumber == 2 & strcmp(modelType,'Selection')),...
		70,plotColor(xindex & sessionNumber == 1 & strcmp(modelType,'Selection'),:),...
		'filled','Marker',selectionMarker,'MarkerEdgeColor','black');
	axis([minParamBound maxParamBound minParamBound maxParamBound]);
	axis square;
	set(gca,'YTick',-20:0.5:20);
	...
	set(gcf, 'PaperOrientation','portrait')
	%print('-dpng',['Figures\paramValues ' cell2str(paramName) ' ' num2str(numParams) ' parameters session correlations.png']);
